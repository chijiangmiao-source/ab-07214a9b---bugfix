...
前端场次看板行内维护 `只读 / 编辑中 / 保存中 / 冲突` 一组草稿状态：网络失败或冲突都保留已输入文本；看板轮询按**场次隔离**归并——切换场次后看板只呈现新场次的完整快照（旧场次行连同行内编辑器立即卸载，不会挂在新场次标题下被误修订），切换后才到达的旧场次轮询/保存响应一律丢弃；同一场次内仍按操作标识归并且修订号只升不降，较早的轮询响应即使晚到也不会覆盖新修订。
...
└── web/                    # React + TypeScript 前端
    ...
    ├── src/lib/board.ts    # 看板轮询归并：场次隔离（切换后只留新场次快照）+ 修订号只升不降
